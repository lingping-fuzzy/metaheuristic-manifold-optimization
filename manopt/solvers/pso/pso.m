function [xbest, fbest, info, options] = pso(problem, pn, itmax, x, options)
...
%     localdefaults.maxiter = max(itmax, 2*dim);
...
%     x = load('specturmFigdraw.x.mat').x;
    ...
    stats = savestats();
    info(1) = stats;
    info(min(1000, options.maxiter+1)).iter = [];
...
%         w = 0.1 + 0.1*(1-iter/options.maxiter); %truncated svd
        % Compute velocities
        for i = 1 : numel(x)
